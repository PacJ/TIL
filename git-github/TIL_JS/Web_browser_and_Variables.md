...
* 웹사이트로 이동시, 서버측으로부터 HTML,CSS,JavaScript 다운로드(Loading)하고 페이지 나타냄.
  ...
  * 파싱중에, HTML의 link 태그를 만나면, DOM tree 생성을 중단하고, link된 CSS 를 Load, Parse 하고 CSSOM tree를 만든한뒤에, 다시 HTML의 파싱과 DOM tree 생성을 재개한다.

  

  * 파싱중에 script 태그를 만나면, 다시 파싱과 DOM tree생성을 중단하고, JavaScript 엔진에게 렌더링 권한을 넘겨 load, parse 하고 Syntax tree를 만든후에 다시 HTML로 파서 권한이 돌아가서 남은 파싱을 완료한다.
...
*  데이터를 재사용하고싶을때, 기억하고싶은 데이터를 메모리에 저장하고, 저장된 데이터를 읽어 들여 재사용하기위해 변수를 사용한다.
*  변수는 하나의 값을 저장할 수 있는 메모리 공간에 붙인 이름, 또는 메모리 공간 자체를 말한다.
   *  memory: 데이터를 저장할 수 있는 memory cell들의 집합체.
      - 셀 하나는 1byte(8bit) 크기. 셀데이터를 저장/읽기(write/read)
      - 모든 데이터의 종류는 2진수
      - 각각 셀은 고유 메모리 주소(memory address)를 가짐.
      - 메모리 주소는 0에서부터 메모리 크기만큼 정수로 표시.
* 변수에 값을 저장하는 것을 할당(assignment 대입/저장)이라 하고, 변수에 저장된 값을 읽어 들이는 것을 참조(reference)라한다.
*  여러개의 연관성 있는 값을 하나로 그루핑하는것: 자료구조
   - 배열, 객체를 사용하면 그룹화하여 하나의 값처럼 사용가능.
...
* 변수 식별자는 사람이 이해할 수 있도록 지정한 이름이기에, 좋은 이름의 변수는 가독성을 높여주기도한다. 
* 식별자 네이밍규칙을 준수하고, 선언(declaration)을 통해 JavaScript engine에 식별자의 존재를 알린다.



## 변수 선언(Variable declaration)
* 변수의 식별자(이름)을 JavaScript engine에 알려서 공간을 확보하는것, 선언할 떄는 var,let,const keyword 사용.(keyword: 자바스크립트 엔진이 수행할 동작을 규정한 명령어)
...
* 초기화단계(undefined 할당)을 거치지 않으면 확보된 메모리 공간에는 이전에 다른 어플이 사용했던 값이 남아있을수있음. 이것을 Garbage value(쓰레기 값)이라 한다.



## 변수 선언의 실행 시점과 변수 호이스팅
* 변수 선언은 소스 코드가 순차적으로 한줄씩 실행되는 시점(run-time)이 아니라 그 전단계인 구문 분석(Syntax analysis)단계에 먼저 실행됨.
* JS engine은 순차적으로 실행(run-time)하기 전 에 먼저 소스 코드 평가 단계를 가지고, 모든 선언문을 찾아내 식별자를 등록하고 초기화후, 선언문을 제외한 소스 코드를 한줄씩 실행(run-time).
* 변수 선언문보다 참조하는 코드들이 앞에 있어도, 선언문이 우선적으로 읽히게된다.
* 변수 선언문이 코드의 선두로 올려진것처럼 동작하는 특징을 변수 호이스팅(variable hoisting)이라 한다.
* 호이스팅이 되긴하지만, 선언문을 참조보다 우선적으로 사용하자.



## 값의 할당

* 자바스크립트에서 7개중 6개의 데이터 타입은 원시값(primitive value)다.
  * 원시값: 값을 변경할수 없다(immutable)
    * 변수를 선언하고 값을 할당할때, 처음 선언할때 할당되는 undefined값을 지우고 그같은 공간에 새로운 값을 할당할 수 없다. 새로운 공간에 값이 들어가게된다.
* 할당 이전에 선언문이 우선적으로 실행된다. 할당과 실행은 순차적으로 run-time에 한다.
...
* 메모리 관리 방식에 따라 Unmanaged Language 와 Managed Language로 분류할수있다.
	* C 언어같은 Unmanaged Language는 개발자가 직접 메모리를 할당하고 해제하기 위해 malloc()과 free()같은 low-level(저수준) 메모리 관리 기능을 제공. 개별자의 역량에 따라 최적의 퍼포먼스를 확보할수있지만, 반대로 치명적 오류를 생산할 가능성도 동시에 존재.
