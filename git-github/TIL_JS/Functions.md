...
* 함수는 함수정의(Function Definition)을 통해 생성한다.
  ...
  * 정의후에 인수를 매개변수를 통해 함수에게 전달해 함수의 실행을지시.
...
* 함수 선언문이 실행되면 함수 객체를 생성하고, 생성된 객체에 할당할 변수가 있어야한다. 함수 객체를 변수에 할당하지 않으면 아무도 참조하지 않게되어 해제된다.
  ...
  * 자바스크립트는 함수이름과 동일한 이름의 식별자를 암묵적으로 선언하고 함수 객체를 할당한다.
...
* 함수는 함수 이름이 아닌 함수 객체를 가르키는 변수로 호출한다. 자바스크립트가 암묵적으로 선언한 함수이름과 동일한 식별자로 호출하는것이다.
...
* 함수가 일급 객체라는것은 함수를 값처럼 자유롭게 사용할 수 있다는 것이다.
...
* 매개변수는 순서에 의미가 있으므로 많이지면 함수를 호출할때 전달해야할 인수의 순서를 고려해야 한다.
